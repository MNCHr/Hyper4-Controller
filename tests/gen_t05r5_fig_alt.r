...
# Load data
t05_data <- read.table("t05r5.dat",header=TRUE,sep=",")
t05p95_data <- read.table("t05r5p95.dat",header=TRUE,sep=",")

# Define colors
plot_colors <- c("black","red")

# Start pdf driver
pdf(file="t05r5.pdf", height=3.5, width=8)
...
# routers removed
abline(v=end-adj,col="forestgreen",lty=3)
draw.circle(x=xpos-adj-1.5,y=6,radius=1,lty=1,lwd=1)
text(x=xpos-adj-1.5, y=6, labels = l)
